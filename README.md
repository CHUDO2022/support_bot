# Support Bot v4 - Инструкция по запуску

## 🚀 Быстрый старт

### Шаг 1: Установка Python

Убедитесь, что у вас установлен Python 3.8 или выше:
```bash
python --version
# или
python3 --version
```

### Шаг 2: Установка зависимостей

Установите зависимости:
```bash
pip install -r requirements.txt
```

### Шаг 3: Настройка конфигурации

1. **Скопируйте `env_example.txt` в `.env`:**
```bash
cp env_example.txt .env
```

2. **Заполните обязательные настройки в `.env`:**

```env
# OpenAI Configuration (ОБЯЗАТЕЛЬНО)
OPENAI_API_KEY=ваш_api_ключ_openai
ASSISTANT_ID=ваш_assistant_id

# Telegram Configuration (ОБЯЗАТЕЛЬНО)
TELEGRAM_API_ID=ваш_telegram_api_id
TELEGRAM_API_HASH=ваш_telegram_api_hash

# Bot Configuration
BOT_NAME=support_bot_v4

# Optional
LOG_LEVEL=INFO
```

#### Где получить настройки:

**OpenAI API:**
1. Зарегистрируйтесь на https://platform.openai.com
2. Перейдите в API Keys и создайте новый ключ
3. Скопируйте ключ в `OPENAI_API_KEY`

**OpenAI Assistant ID:**
1. Создайте Assistant в OpenAI Platform
2. Скопируйте Assistant ID в `ASSISTANT_ID`

**Telegram API:**
1. Перейдите на https://my.telegram.org/apps
2. Войдите с номером телефона
3. Создайте приложение
4. Скопируйте `api_id` и `api_hash`

### Шаг 4: Запуск бота

Просто запустите:
```bash
python start.py
```

Или на Linux/Mac:
```bash
python3 start.py
```

---

## 📋 Структура проекта

```
project/
├── bot/                       # Работа с ботом
│   ├── main.py               # Главный файл бота
│   ├── behavior.py           # Поведение бота
│   └── filters.py            # Фильтры сообщений
├── ai/                       # Работа с ИИ
│   ├── detector.py           # Детектор запросов
│   └── cleaner.py            # Очистка текста
├── files/                    # Работа с файлами
│   └── manager.py            # Менеджер файлов
├── storage/                  # Хранилища
│   ├── threads.py            # Хранилище threads
│   └── state.py              # Хранилище состояния
├── config/                   # Конфигурация
│   └── settings.py           # Настройки
├── start.py                  # Точка входа
├── requirements.txt          # Зависимости
├── env_example.txt           # Пример конфигурации
└── .env                      # Ваши настройки (создается вручную)
```

---

## ⚠️ Важные замечания

1. **Файл `.env` обязателен** - без него бот не запустится
2. **API ключи должны быть валидными** - проверьте их перед запуском
3. **При первом запуске Telegram** попросит ввести код подтверждения
4. **Логи сохраняются** в файл `bot.log`

---

## 🔧 Решение проблем

### Ошибка "OPENAI_API_KEY не найден"
- Убедитесь, что файл `.env` существует
- Проверьте, что переменная `OPENAI_API_KEY` заполнена в `.env`

### Ошибка "ModuleNotFoundError"
- Установите зависимости: `pip install -r requirements.txt`

### Ошибка при запуске Telegram
- Проверьте правильность `TELEGRAM_API_ID` и `TELEGRAM_API_HASH`
- Убедитесь, что номер телефона указан корректно

---

## 📞 Поддержка

При возникновении проблем проверьте логи в файле `bot.log`

---

## 📤 Загрузка на GitHub

### Инициализация Git репозитория

```bash
git init
git add .
git commit -m "Initial commit: Support Bot v4 refactored"
```

### Создание репозитория на GitHub

1. Перейдите на https://github.com
2. Нажмите "New repository"
3. Укажите имя репозитория (например: `support-bot-v4`)
4. **НЕ** добавляйте README, .gitignore или лицензию (они уже есть)
5. Нажмите "Create repository"

### Подключение и загрузка

```bash
git remote add origin https://github.com/ВАШ_USERNAME/ВАШ_РЕПОЗИТОРИЙ.git
git branch -M main
git push -u origin main
```

**Важно:** Файл `.env` **НЕ** будет загружен (он в `.gitignore`). Все секретные данные защищены.
